...
function hash_call($methodName,$nvpStr) {
	...
	$API_CertPath = PP_API_CERTPATH;
	$subject = PP_SUBJECT;
	
	//setting the curl parameters.
	$ch = curl_init();
	
	// Find the proper auth mode
	if(!empty($API_UserName) && !empty($API_Password) && !empty($API_CertPath)) {
		$AuthMode = "CERT";
	} elseif(!empty($API_UserName) && !empty($API_Password) && !empty($API_Signature) && !empty($subject)) {
		$AuthMode = "THIRDPARTY";
	...
	switch($AuthMode) {	
		case "CERT" : 
				$nvpHeader = "&PWD=".urlencode($API_Password)."&USER=".urlencode($API_UserName);
				curl_setopt($ch, CURLOPT_SSLCERT, $API_CertPath);
				break;
		case "3TOKEN" : 
